...
> **Note (2026-04-21):** Centralized SaaS architecture-д тааруулсан.
> `edge_boxes`, `edge_box_metrics`, `sync_packs` table-ууд dormant
> (on-prem SKU-д reserved). Шинэ `tenants`-oriented schema + shared
> behavior taxonomy Qdrant collection нэмсэн. See
...
> болон
> [`07-SCHEMA-MIGRATION-LOCK.md`](./07-SCHEMA-MIGRATION-LOCK.md).

Одоогийн DB schema-д нэмэх шинэ хүснэгтүүд, хувьсалын migration план,
TimescaleDB integration, Qdrant collections.
...
### 2.1 `edge_boxes` (dormant)

> **Status (2026-04-21):** DORMANT in default SaaS deployment. Schema
> retained for optional on-prem SKU
> ([`decisions/2026-04-21-drop-edge-box-hybrid-architecture.md`](./decisions/2026-04-21-drop-edge-box-hybrid-architecture.md)).
> New SaaS tenants never insert rows here. Do not drop — migration
> churn cost > benefit.

Edge box бүрийг тусад нь бүртгэнэ (on-prem SKU only).
...
### 2.6 `sync_packs` (dormant)

> **Status (2026-04-21):** DORMANT in default SaaS deployment (no edge
> → central sync). Reserved for on-prem SKU versioning. See header
...
PostgreSQL-ийн гадна Qdrant vector DB-д хоёр түвшний collection-ууд
ажиллана: **per-tenant (isolated)** болон **shared (anonymized
taxonomy)**.

### 5.1 Per-tenant (isolated)

Tenant бүрт тусдаа collection. RBAC-аар tenant цэцэгчлэсэн service
account зөвхөн өөрийн collection-д хандана.

#### `tenant_{tenant_id}_case_memory`

Tenant-specific case memory (RAG Layer 2).
...
# Payload
{
    ...
    "alert_id": BigInt,
    "camera_id": int,
    "tenant_id": UUID,  # same as collection suffix; dup for filtering
    "timestamp": ISO8601,
    "label": "confirmed_theft" | "false_positive" | "unclear",
    "confidence": float,
    "behavior_scores": dict,
    "vlm_verdict": str,  # VLM-ийн шалтгаан тайлбар (optional)
}
```

**Legacy note:** Previously named `store_{store_id}_cases`. Rename
migration-ыг schema lock-ийн дагуу pre-2026-04-21 deployment-д
dual-read/backfill-оор хийнэ.

#### `tenant_{tenant_id}_reid_gallery`

Re-ID persistent gallery (30-day TTL default, tenant opt-out нь TTL=0).
...
# Payload
{
    "person_reid_id": UUID,  # stable person identifier within tenant
    "tenant_id": UUID,
    "camera_ids_seen": [int, ...],
    "first_seen_at": ISO8601,
    "last_seen_at": ISO8601,
    "seen_count": int,
    "confirmed_alerts_count": int,  # хэдэн confirmed alert-тэй холбоотой
}
```

**Legacy note:** Previously `store_{store_id}_reid` with 2h TTL.
Extended to 30-day per [`09-PRIVACY-LEGAL.md`](./09-PRIVACY-LEGAL.md)
retention policy (tenant-configurable, GDPR-aligned).

### 5.2 Shared (cross-tenant, anonymized)

#### `behavior_taxonomy_v1`

Хулгайлах загваруудын global анонимат санал — бүх tenant-аас
contribution нэмэгдэж, бүх tenant-д benefit өгнө. **No PII, no
tenant_id, no image/clip.**

```python
VectorParams(
    size=512,  # MotionBERT-style pose trajectory embedding
    distance=Distance.COSINE,
)

# Payload (strict schema — anonymization tests enforce)
{
    "pattern_id": UUID,
    "pattern_type": str,  # "loitering_theft", "distraction_team",
                          # "staff_restocking", "concealment_pocket",
                          # "concealment_bag", ...
    "temporal_window_sec": int,  # e.g. 5, 10, 15
    "pose_feature_dim": 512,
    "confirmed_by_tenant_count": int,  # хэдэн tenant-аас батлагдсан
    "first_seen_at": ISO8601,
    ...
    # - person_reid_id
    # - any image reference
    # - any user/timestamp trace
}
```

**Write rule (enforced in app layer + Qdrant trigger):**
- Caller must be `taxonomy_writer_role` service account.
- Payload schema validated against `anonymization_test_harness`.
- Opted-out tenant-ийн event bol write бүх давхаргад rejected.

**Read access:**
- All tenant service accounts read-only.
- Used in Layer 2 RAG дуудаж scoring-д blend хийнэ.

### 5.3 Collection lifecycle management

| Event | Action |
|---|---|
| Шинэ tenant онбординг | `provision-tenant.sh` үүсгэнэ. TTL + HNSW params set. |
| Tenant off-boarding | 30 хоногийн grace-ийн дараа `qdrant delete-collection`. |
| Tenant opt-out shared taxonomy | App-layer flag; Qdrant-д dedicated action шаардагдахгүй. |
| Sharded growth (>10M vectors) | HNSW `m=32, ef_construct=200` + сегментжүүлэлт. Phase C-д. |

### 5.4 Backup schedule

- Per-tenant collection: nightly snapshot → S3 (per-tenant encryption key).
- `behavior_taxonomy_v1`: hourly snapshot + versioned. Critical for
  product moat.
...
- [03-TECH-SPECS.md](./03-TECH-SPECS.md) §14 (Shared taxonomy)
- [05-ONBOARDING-PLAYBOOK.md](./05-ONBOARDING-PLAYBOOK.md) — Tenant provisioning
- [07-SCHEMA-MIGRATION-LOCK.md](./07-SCHEMA-MIGRATION-LOCK.md)
- [09-PRIVACY-LEGAL.md](./09-PRIVACY-LEGAL.md) — Re-ID retention, anonymization
- [audits/multi-tenant-isolation-2026-04-21.md](./audits/multi-tenant-isolation-2026-04-21.md) — Isolation audit
- [decisions/2026-04-21-tenant-isolation-strategy.md](./decisions/2026-04-21-tenant-isolation-strategy.md)

### Superseded

- `05-MIGRATION-PLAN.md` — archived (hybrid → central migration)
